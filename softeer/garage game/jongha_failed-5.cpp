...
void dfs2(vector<vector<long long>> tempv, int level, long long tempresult) {
    if (level == 3) {
...
void dfs(vector<vector<long long>> tempv, int level, long long tempresult) {
        ...
        for (int i = height - 1; i >= realh; i--) {
            for (int j = 0; j < n; j++) {
                if (visited[i][j] == 1) {
                    ...
                    int a = 1;
                    int b = 0;
                    int end; int start = height;
                    while (1) {
                        end = i - a;
                        if (start < realh) break;
                        if (end < 0) break;
                        if (visited[end][j] == 1) {
                            visited[end][j] = 0;
                            a++;
    ...
    for (int i = realh; i < height; i++) {
        for (int j = 0; j < n; j++) {
            ...
            if (level == 2) {
                vector<vector<long long>> tmpv(n, vector<long long>(n, 0));

                for (int b = 0; b < n; b++) {
                    for (int l = 0; l < n; l++) {
                        tmpv[b][l] = tempv[realh + b][l];
                    }
                }
                dfs2(tmpv, level + 1, tempresult + ((rd.y - lu.y + 1) * (rd.x - lu.x + 1) + cnt));
            }
            else {
