...
void bfs(YX st, int st_num) {
    ...
    for (int i = 0; i < locate.size(); i++) {
        if (temp[locate[i].y][locate[i].x] != 0) {
            pq.push({ st_num, i , temp[locate[i].y][locate[i].x] });
        }
    }
}
void setting() {
    ...
    //        int temp_price = bfs(st, end); // 여기에서 bfs(st,end)만 놔두고 싹다 bfs안으로 옮겨서 실행될때 다른 a,s값들price 대입
    //        pq.push({ i, j, temp_price });
    //    }
    //}
    for (int i = 0; i < locate.size(); i++) {
        YX st = { locate[i].y,locate[i].x };
        bfs(st,i);
    }
}
